...
from Base_Model.MTCNN_nets import ONet
...
def train(model, path, epoch=10):
    ...
    for epoch in range(num_epochs):
        ...
        print('accuracy: {:.4f} loss: {:.4f} cls Loss: {:.4f} offset Loss: {:.4f} landmark Loss: {:.4f}'
              .format(epoch_accuracy, epoch_loss, epoch_loss_cls, epoch_loss_offset, epoch_loss_landmark))
...
def prune_model(model, prunner, path):
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        with torch.set_grad_enabled(True):
            pred_landmark, pred_offsets, pred_label = prunner.forward(input_images)
...
def get_args():
    ...
    parser.add_argument("--train_path", type = str, default = "../data_preprocessing/anno_store/imglist_anno_48.txt")
    parser.add_argument("--test_path", type = str, default = "../data_preprocessing/anno_store/imglist_anno_48_val.txt")
    parser.add_argument("--filter_size", type = int, default = 36)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("Base_Model/onet_Weights", map_location=lambda storage, loc: storage))
    
    prunner = FilterPrunner(model, use_cuda = True) 
    
    save_dir = 'saving_onet_prunning_result'
    ...
    since = time.time()
    accuracy, loss, loss_cls, loss_offset, loss_landmark = test(model, args.test_path)
    print('initial test :: accuracy: {:.4f} loss: {:.4f} cls loss: {:.4f} offset loss: {:.4f} landmark loss: {:.4f}'
          .format(accuracy, loss, loss_cls, loss_offset, loss_landmark))
    ...
    for it in range(iterations):
        ...
        accuracy, loss, loss_cls, loss_offset, loss_landmark = test(model, args.test_path)
        print('iter{}. without retrain :: accuracy: {:.4f} loss: {:.4f} cls loss: {:.4f} offset loss: {:.4f} landmark loss: {:.4f}'
              .format(it, accuracy, loss, loss_cls, loss_offset, loss_landmark))
        ...
        since = time.time()
        accuracy, loss, loss_cls, loss_offset, loss_landmark = test(model, args.test_path)
        print('iter{}. after retrain :: accuracy: {:.4f} loss: {:.4f} cls loss: {:.4f} offset loss: {:.4f} landmark loss: {:.4f}'
              .format(it, accuracy, loss, loss_cls, loss_offset, loss_landmark))
        print("iter{}. test time cost is {:.2f} s".format(it, time.time()-since))
        
        torch.save(model.state_dict(), os.path.join(save_dir, 'onet_weights_pruned_{}'.format(it)))
        torch.save(model, os.path.join(save_dir, 'onet_prunned_{}'.format(it)))
