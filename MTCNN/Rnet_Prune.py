...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 28 14:14:28 2019
...
from Base_Model.MTCNN_nets import RNet
...
def get_args():
    ...
    parser.add_argument("--train_path", type = str, default = "../data_preprocessing/anno_store/imglist_anno_24.txt")
    parser.add_argument("--test_path", type = str, default = "../data_preprocessing/anno_store/imglist_anno_24_val.txt")
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("Base_Model/rnet_Weights", map_location=lambda storage, loc: storage))
    
    prunner = FilterPrunner(model, use_cuda = True) 
    
    save_dir = 'saving_rnet_prunning_result'
    ...
    for it in range(iterations):
        ...
        torch.save(model.state_dict(), os.path.join(save_dir, 'rnet_weights_pruned_{}'.format(it)))
        torch.save(model, os.path.join(save_dir, 'rnet_prunned_{}'.format(it)))
